(() => {
  ...
  class ChatHistoryUI {
    ...
    createHistoryItemElement(item, currentThreadId) {
      ...
      historyItem.setAttribute('data-thread-id', item.threadId);
      ...
      historyItem.innerHTML = `
        <div class="flex justify-between items-start group">
          ...
          <div class="flex items-center space-x-1 ml-2">
            ...
            <button class="delete-btn opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-700 p-1 rounded transition-opacity duration-200" data-thread-id="${item.threadId}">
    ...
    removeHistoryItem(container, threadId) {
      ...
      const item = container.querySelector(`.chat-item[data-thread-id="${threadId}"]`);
      if (item) {
        item.remove();
      }
    }
    ...
    updateActiveState(container, threadId, isActive) {
      ...
      const items = container.querySelectorAll('.chat-item');
      items.forEach(item => {
        const itemThreadId = item.dataset.threadId;
